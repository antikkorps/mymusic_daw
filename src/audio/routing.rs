...
impl AudioRoutingGraph {
    ...
    pub fn process(&mut self) -> (f32, f32) {
        ...
        if self.processed_order.is_none() {
            if let Err(e) = self.compute_topological_order() {
        ...
        if let Some(order) = &self.processed_order {
            ...
            if let Some(output_id) = self.get_output_node_id() {
                if let Some(outputs) = node_outputs.get(&output_id) {
                    if let Some((left, right)) = outputs.get("main") {
                        return (*left, *right);
                    }
                }
            }
    ...
    fn has_path_dfs(
        ...
        for connection in &self.connections {
            if connection.from_node == current {
                if self.has_path_dfs(connection.to_node, target, visited, stack) {
                    return true;
                }
            }
        }
