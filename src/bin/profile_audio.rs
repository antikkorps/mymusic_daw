...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Find bottlenecks
    if let Some((bottleneck_op, bottleneck_stats)) = stats
        .operation_stats
        .iter()
        .max_by_key(|(_, stats)| stats.total_time)
    {
        let bottleneck_percent =
            (bottleneck_stats.total_time as f64 / stats.total_callback_time as f64) * 100.0;
        println!(
            "🔍 Primary bottleneck: {} ({:.1}% of total time)",
            bottleneck_op, bottleneck_percent
        );
    }
    ...
    if stats.operation_stats.contains_key("audio_generation") {
        ...
        let audio_gen_percent =
            (audio_gen_stats.total_time as f64 / stats.total_callback_time as f64) * 100.0;
    ...
    if stats.operation_stats.contains_key("plugin_processing") {
        ...
        let plugin_percent =
            (plugin_stats.total_time as f64 / stats.total_callback_time as f64) * 100.0;
