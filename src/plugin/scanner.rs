...
impl PluginScanner {
    ...
    pub fn scan_directory(&mut self, dir_path: &Path) -> PluginResult<Vec<PluginDescriptor>> {
        ...
        for entry in entries {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("clap") {
                if let Ok(descriptor) = self.scan_file(&path) {
    ...
    pub fn scan_file(&mut self, file_path: &Path) -> PluginResult<PluginDescriptor> {
        ...
        if self.blacklist.iter().any(|blacklisted| file_path_str.contains(blacklisted)) {
    ...
    pub fn search_by_name(&self, name: &str) -> Vec<&PluginDescriptor> {
        self.cache
            .values()
            .filter(|entry| entry.descriptor.name.to_lowercase().contains(&name.to_lowercase()))
    ...
    pub fn search_by_vendor(&self, vendor: &str) -> Vec<&PluginDescriptor> {
        self.cache
            .values()
            .filter(|entry| entry.descriptor.vendor.to_lowercase().contains(&vendor.to_lowercase()))
            .map(|entry| &entry.descriptor)
            .collect()
    }
...
pub fn get_default_search_paths() -> Vec<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        paths.push(dirs::home_dir().unwrap_or_default().join("Library/Audio/Plug-Ins/CLAP"));
    }

    #[cfg(target_os = "windows")]
    {
        if let Some(program_files) = std::env::var_os("ProgramFiles") {
            paths.push(PathBuf::from(program_files).join("Common Files").join("CLAP"));
        }
        if let Some(program_files_x86) = std::env::var_os("ProgramFiles(x86)") {
            paths.push(PathBuf::from(program_files_x86).join("Common Files").join("CLAP"));
        }
...
mod tests {
    ...
    fn test_default_search_paths() {
        ...
        #[cfg(target_os = "macos")]
        {
            assert!(paths.iter().any(|p| p.to_string_lossy().contains("Library/Audio/Plug-Ins/CLAP")));
        }
    }
